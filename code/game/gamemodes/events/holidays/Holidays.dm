...
/proc/Get_Holiday()
	...
	switch(MM)
		if(1)	//Jan
			switch(DD)
				if(1)
					GLOB.Holiday["New Years's Day"] = "The day of the new solar year on Sol."
				if(12)
					GLOB.Holiday["Vertalliq-Qerr"] = "Vertalliq-Qerr, translated to mean 'Festival of the Royals', is a \
					...
					elaborate displays."
				if(14)
					GLOB.Holiday["Lohri"] = "A human festival traditionally celebrating the end of winter on the Indian subcontinent. \
				...
				if(30)
					GLOB.Holiday["Lunar New Year"] = "Originally the new year on the ancient lunisolar calendar, the Lunar New Year is \
		...
		if(2)	//Feb
			switch(DD)
				if(2)
					GLOB.Holiday["Groundhog Day"] = "An unoffical holiday based on medieval folklore that originated on Earth, \
				...
				if(14)
					GLOB.Holiday["Valentine's Day"] = "A human holiday that revolves around expressions of romance and love. \
					In particular, the exchanging of gifts, letters and cards is traditional."
				if(15)
					GLOB.Holiday["Lantern Festival"] = "A human holiday with origins in Chinese new year celebrations. Participants \
				...
				if(17)
					GLOB.Holiday["Random Acts of Kindness Day"] = "An unoffical holiday that challenges everyone to perform \
		...
		if(3)	//Mar
			switch(DD)
				...
				if(17)
					GLOB.Holiday["St. Patrick's Day"] = "A holiday originating on Earth, celebrating a popular version of Irish culture. \
					Traditions include elaborate parades, wearing of the colour green, and drinking alcohol."
				if(18)
					GLOB.Holiday["Holi"] = "Also known as the Festival of Colours, a human Hindu festival celebrating divine love and the \
				...
				if(27)
					GLOB.Holiday["Easter"] = "A Earth springtime festival variously celebrating rebirth and the beginning of the planting \
		...
		if(4)	//Apr
			switch(DD)
				if(1)
					GLOB.Holiday["April Fool's Day"] = "A human holiday that endevours one to pull pranks and spread hoaxes on their friends."
				if(5)
					GLOB.Holiday["First Day of Passover"] = "The first of eight days of a human holiday celebrating the exodus of ancient Jewish people \
				...
				if(22)
					GLOB.Holiday["Earth Day"] = "A holiday of enviromentalism, that originated on it's namesake, Earth."
		...
		if(5)	//May
			switch(DD)
				...
				if(28)
					GLOB.Holiday["Jiql-tes"] = "A skrellian holiday that translates to 'Day of Celebration', skrell communities \
					gather for a grand feast and give gifts to friends and close relatives."

		if(6)	//Jun
			switch(DD)
				if(6)
					GLOB.Holiday["Sapient Rights Day"] = "This holiday celebrates the passing of the Declaration of Sapient Rights by SolGov, which guarantees the \
					same protections humans are granted to all sapient, living species."
				if(14)
					GLOB.Holiday["Blood Donor Day"] = "This holiday was created to raise awareness of the need for safe blood and blood products, \
					and to thank blood donors for their voluntary, life-saving gifts of blood."
				if(20)
					GLOB.Holiday["Civil Servant's Day"] = "Civil Servant's Day is a holiday observed in SCG member states that honors civil servants everywhere,\
...
/*				if(25)
					GLOB.Holiday["Merhyat Njarha"] = "A Njarir'Akhan tajaran tradition translating to \"Harmony of the House\", in which Njarjirii citizens pay \
		...
		if(7)	//Jul
			switch(DD)
				if(1)
					GLOB.Holiday["Doctors' Day"] = "A holiday that recognizes the services of physicians, commonly celebrated \
		...
		if(8)	//Aug
			switch(DD)
				...
				if(20)
					GLOB.Holiday["Obon"] = "An ancient Earth holiday originating in east Asia, for the honouring of one's ancestral spirits. \
					Traditions include the maintenance of grave sites and memorials, and community traditional dance performances."
				if(27)
					GLOB.Holiday["Forgiveness Day"] = "A time to forgive and be forgiven."

		if(9)	//Sep
			switch(DD)
				if(17)
					GLOB.Holiday["Qill-xamr"] = "Translated to 'Night of the dead', it is a skrellian holiday where skrell \
				...
				if(19)
					GLOB.Holiday["Talk-Like-a-Pirate Day"] = "Ahoy, matey! It be the unoffical holiday celebratin' the salty \
					sea humor of speakin' like the pirates of old."
				if(20)
					GLOB.Holiday["Rosh Hashanah"] = "An old human holiday that marks the traditional Hebrew new year."
				if(28)
					GLOB.Holiday["Stupid-Questions Day"] = "Known as Ask A Stupid Question Day, it is an unoffical holiday \
					created by teachers in Sol, very long ago, to encourage students to ask more questions in the classroom."

		if(10)	//Oct
			switch(DD)
				if(9)
					GLOB.Holiday["Lief Eriksson Day"] = "A day commemorating Norse explorer Lief Eriksson, an early Scandinavian cultural figure \
					who is thought to have been the first European to set foot in North America."
				if(16)
					GLOB.Holiday["Boss' Day"] = "Boss' Day has traditionally been a day for employees to thank their bosses for the difficult work that they do \
					throughout the year. This day was created for the purpose of strengthening the bond between employer and employee."
				if(21)
					GLOB.Holiday["First Day of Diwali"] = "An ancient Hindu, Jain and Sikh festival lasting five days, celebrating victory of light over darkness, good over \
				...
				if(31)
					GLOB.Holiday["Halloween"] = "Originating from Earth, Halloween is also known as All Saints' Eve, and \
		...
		if(11)	//Nov
			switch(DD)
				if(1)
					GLOB.Holiday["Day of the Dead"] = "An old human holiday celebrating the lives of deceased friends and family members, \
				...
				if(13)
					GLOB.Holiday["Kindness Day"] = "Kindness Day is an unofficial holiday to highlight good deeds in the \
		...
		if(12)	//Dec
			switch(DD)
				if(10)
					GLOB.Holiday["Human-Rights Day"] = "An old holiday created by an intergovernmental organization known back than as the United Nations, \
					...
					reminder."
				if(22)
					GLOB.Holiday["Vertalliq-qixim"] = "A skrellian holiday that celebrates the skrell's first landing on one of \
					their moons. It's often celebrated with grand festivals."
				if(24)
					GLOB.Holiday["Christmas Eve"] = "The eve of Christmas, an old holiday from Earth that mainly involves gift \
					giving, decorating, family reunions, and a fat red human breaking into people's homes to steal milk and cookies."
				if(25)
					GLOB.Holiday["Christmas"] = "Christmas is a very old holiday that originated in Earth, Sol.  It was a \
				...
				if(31)
					GLOB.Holiday["New Year's Eve"] = "The eve of the New Year for Sol.  It is traditionally celebrated by counting down to midnight, as that is \
